...
def index():
    ...
    if request.method == "GET":
        # Gets the necesary data from the user according to its ID from the database

        # Gets the user's username
        username = db.execute("SELECT username FROM users WHERE id = :id", id=session["user_id"])
        # Gets the user's cash
        user_cash = db.execute("SELECT cash FROM users WHERE id = :id", id=session["user_id"])
        user_cash = user_cash[0]["cash"]

        # Gets the users shares (symbol and total ammount) and using the search function updates it with the current price and total value
...
def buy():
    if request.method == "GET":
        ...
        # Ensures that the user has enough money to make the purchase, else returns an apology
        ...
        # Makes the purchase
            ...
            # Stores the correspondant data in the 'portfolio' table inside the finance database
            ...
            # Updates the users cash in the 'users' table inside the finance database
...
def register():
    ...
    if request.method == "POST":
        ...
        # Stores user id in the session to keep it logued in
...
def sell():
    if request.method == "GET":
        ...
        if ammount_user_wants_to_sell <= 0:
            return apology("You must a number greater than zero", 400)
        if not lookup(symbol):
            return apology("Incorrect symbol", 400)

        # Checks if the user owns the inputed stock and returns the ammount it owns (if any)
        ...
        # Checks the user has enough quantity to sell
        if user_stocks[0]["SUM(quantity)"] < ammount_user_wants_to_sell:
            return apology("You are trying to sell more than you own", 400)

        # Updates the database with the new ammount of shares and the money adquired after selling

        # Updates the users current cash acording to the ammount earn after the transaction
        ...
        # Updates the ammount of stocks the user has now
        db.execute("INSERT INTO portfolio (id, symbol, quantity, price, date) VALUES (:id, :symbol, :quantity, :price, :date)",
...
def change_password():
    if request.method == "GET":
        ...
        # Makes the new changes in the database
